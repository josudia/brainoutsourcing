...
Complete guide to effectively using MCP tools in Claude Code plugin commands and agents.
...
- `mcp__plugin_myplugin_database__query`
- `mcp__plugin_myplugin_database__execute`
- `mcp__plugin_myplugin_database__list_tables`
...
- Full tool names for configuration
...
To create a task:
1. Collect task details from the user
...
**Caution:** Only use wildcards when the command truly needs access to all tools from a server.
...
2. Specify search filters (assignee, project, etc.)
...
Agents can use MCP tools autonomously without pre-allowing:
...
3. **Generate report**: Create a formatted status update
4. **Update Asana**: Use mcp__plugin_asana_asana__asana_create_comment to post the report

## Available Tools

The agent has access to all Asana MCP tools without prior approval.
```

### Agent Tool Access

Agents have broader tool access than commands:
- Can use any tool that Claude deems necessary
- Do not require pre-allowed lists
...
```markdown
Steps:
1. Validate that the user provided required fields
...
Multiple calls with the same tool:
...
2. On error (rate limit, network, etc.):
...
Each MCP tool has a schema that defines its parameters. View with `/mcp`.
...
Claude automatically structures tool calls based on the schema:
...
**Validate in commands before calling:**
...
1. Check required parameters:
   ...
   - Due date has valid format (YYYY-MM-DD)
2. If validation fails, prompt user to provide missing data
...
2. On error:
   ...
   - Do not show internal error details to the user
```
...
3. Report summary:
   - "8 of 10 items processed successfully"
...
4. Only re-fetch when data changes
```

### Parallel Tool Calls

When tools do not depend on each other, call them in parallel:

```markdown
Steps:
1. Parallel calls (Claude handles this automatically):
...
4. Provide completion summary at the end
```
...
**Document MCP tool usage in the command:**
```markdown
## MCP Tools Used

This command uses the following Asana MCP tools:
- **asana_search_tasks**: Search tasks by criteria
- **asana_create_task**: Create a new task with details
- **asana_update_task**: Update existing task properties

Ensure you are authenticated with Asana before running this command.
```
...
```markdown
Steps:
1. Create test data in the external service
...
4. Verify graceful error handling works
```
...
## Delete
Use delete_item with item ID (get confirmation first)...
```
...
4. **Present**: Format and display to the user
```
...
```markdown
Steps:
1. **Preparation**: Gather all required information
2. **Validation**: Check data completeness
3. **Execution**: Chain of MCP tool calls:
   - Create parent resource
   - Create child resources
   - Link resources
   - Add metadata
4. **Verification**: Confirm all steps succeeded
5. **Report**: Provide summary to the user
```
...
- Server connected (check with `/mcp`)
...
**Check:**
- Batch queries instead of calling individually
...
1. **Understand tool schemas** via `/mcp`
2. **Pre-allow tools in commands** appropriately
3. **Handle errors gracefully**
4. **Optimize performance** with batching and caching
5. **Provide good UX** with feedback and clear errors
